...
WORKDIR /app/service
COPY leasebase-maintenance-service/package.json leasebase-maintenance-service/package-lock.json* ./
RUN npm ci
COPY leasebase-maintenance-service/tsconfig.json ./
COPY leasebase-maintenance-service/src ./src
RUN npm run build
